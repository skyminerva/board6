...
<body>  
	<div style="text-align:center">
		...
		<div style="text-align:right; width: 89.5%;"> 
			...
			<button onclick="location.href='/board/freeInsert'">자유 게시판 등록</button>
		...
		<thead>
			...
			<c:forEach var="freeBoard" items="${freeBoard}">
				<tr >
					<td width="10%"><a href="/board/select?id=${freeBoard.id}">${freeBoard.id}</a></td>
					<td width="10%"><a href="/board/select?id=${freeBoard.id}">${freeBoard.name}</a></td>
					<td width="30%">${freeBoard.title}</td>
					<td width="15%"><fmt:formatDate pattern="yyyy-MM-dd" value="${freeBoard.regdate}" /></td>
				</tr>
			</c:forEach>
		</table>
	</div>
