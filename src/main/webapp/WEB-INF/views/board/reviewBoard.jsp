...
<body>  
	<div style="text-align:center">
		...
		<div style="text-align:right; width: 89.5%;"> 
			...
			<button onclick="location.href='/board/reviewInsert'">review 게시판 등록</button>
		...
		<thead>
			...
			<c:forEach var="reviewBoard" items="${reviewBoard}">
				<tr >
					<td width="10%"><a href="/board/select?id=${reviewBoard.id}">${reviewBoard.id}</a></td>
					<td width="10%"><a href="/board/select?id=${reviewBoard.id}">${reviewBoard.name}</a></td>
					<td width="30%">${reviewBoard.title}</td>
					<td width="15%"><fmt:formatDate pattern="yyyy-MM-dd" value="${reviewBoard.regdate}" /></td>
