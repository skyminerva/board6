...
<body>  
	<div style="text-align:center">
		...
		<div style="text-align:right; width: 89.5%;"> 
			...
			<button onclick="location.href='/board/javaInsert'">자바 게시판 등록</button>
		...
		<thead>
			...
			<!-- 게시글 리스트 -->
			<c:forEach var="javaBoard" items="${javaBoard}">
				<tr >
					<td width="10%"><a href="/board/select?id=${javaBoard.id}">${javaBoard.id}</a></td>
					<td width="10%"><a href="/board/select?id=${javaBoard.id}">${javaBoard.name}</a></td>
					<td width="30%">${javaBoard.title}</td>
					<td width="15%"><fmt:formatDate pattern="yyyy-MM-dd" value="${javaBoard.regdate}" /></td>
